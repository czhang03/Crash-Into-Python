...
def filter_index_operation(condition, lst):
    ...
    res = []
    for elem in lst:
        # if the condition is satisfied, then add the element
        if condition(elem):
            res.append(elem)
    return res
...
def filter_using_foldl(condition, lst):
    ...
    return foldl(
        start=[],
        # add element to the res if the condition is satisfied
        operation=lambda res, elem: res + [elem] if condition(elem) else res,
        lst=lst
    )
...
def all_using_index(bool_list):
    ...
    for bool_val in bool_list:
        res = res and bool_val
    return res


# IMPORTANT
# there is another way
def all_using_index_another(bool_list):
    """Implement the function all using index operation

    Python has a built in function called `all` that behave the same as this function
    This checks if a list is all True

    >>> all_using_index_another([True, True, False, True])
    False
    >>> all_using_index_another([True, True])
    True
    >>> all_using_index_another([False, False])
    False
    ...
    """
    for bool_val in bool_list:
        # if any of the element is false
        # then return False and immediately terminate the function
        # (in Python, when the execution hits a return statement, the function immediately terminates)
        # *IMPORTANT*:
        # 1. return statement in the middle of the function are VERY VERY BAD.
        #   It can easily create untraceable code if your function is long.
        #   return statement should ALWAYS be at the end of the function.
        # 2. using return statement in the middle of your function will most likely
        #   NOT improve the performance (there are always clean way to make the function as efficient)
        # 3. the `is` keyword is almost like ==, but more strict
        #   For example in python, for some bazaar reason, `1 == True` is true,
        #   and `1 is True` is false (which is the behavior we want.)
        if bool_val is False:
            return False
    # if the function haven't terminated yet,
    # then there is no False in the list
    return True
...
def all_using_foldl(bool_list):
    ...
    return foldl(
        start=True,
        operation=lambda res, elem: res and elem,
        lst=bool_list
    )
...
def all_using_any(bool_list):
    ...
    # forall P = neg exists neg P
    return not any([not elem for elem in bool_list])
...
def zip_using_zip_with(lst1, lst2):
    ...
    """
    return zip_with(lambda a, b: (a, b), lst1, lst2)
...
def zip3_using_zip(lst1, lst2, lst3):
    ...
    :return: a list of tuple containing the corresponding element of `lst1` and `lst2`
    """
    return [(a, b, c) for (a, (b, c)) in zip(lst1, zip(lst2, lst3))]
...
def is_elem(elem, lst):
    ...
    """
    return any([elem == lst_elem for lst_elem in lst])
...
def find_all(elem, lst):
    ...
    """
    return [index for (index, lst_elem) in enumerate(lst) if lst_elem == elem]
...
def slicing(lst, start, end):
    ...
    """
    # `start <= index < end` is a syntactic sugar for `start <= index and index < end`
    return [lst_elem for (index, lst_elem) in enumerate(lst) if start <= index < end]
...
def drinking_game(student_matrix):
    ...
    """
    # `is` is a stricter equality checking in python,
    # that will garentee that two things you are comparing are "exactly" the same
    cs_student_total_score = [4 * wine + 1 * beer for (name, major, beer, wine) in student_matrix if major is "CS"]
    math_student_total_score = [4 * wine + 1 * beer for (name, major, beer, wine) in student_matrix if major is "Math"]

    # return the winning team
    if cs_student_total_score < math_student_total_score:
        return "Math"
    elif cs_student_total_score > math_student_total_score:
        return "CS"
    elif cs_student_total_score == math_student_total_score:
        return "Draw"
    else:
        # this case will be reached if > < and == all did not match the result
        # which is possible if some of them is ill-defined
        # this is my personal preference, I always write a raise exception in the else block
        raise Exception("comparision operation is ill defined with ",
                        cs_student_total_score, " and ", math_student_total_score)
